...
if __name__ == '__main__':
    ...
    # train_path = "./input/train_random_pseudo_label.csv"
    ...
    #
    # # ----------------Read Test Data----------------------------------
    # raw_test = pd.read_csv(test_path, encoding="utf-8", usecols=[1])
    ...
    #
    # raw_test["Content"] = Parallel(n_jobs=NUM_CORES, verbose=10)(
    #     delayed(del_stop_words)(raw_test.iloc[index]["Content"], stop_words) for index in range(len(raw_test)))
    #
    ...
    #
    # raw_test = raw_test.dropna(subset=["Content"])
    ...
    # del raw_train
    #
    # x_test = raw_test["Content"].values
    ...
    # # all_text = np.append(x_train)
    # count_v0 = CountVectorizer()
    # counts_all = count_v0.fit_transform(x_train)
    #
    ...
    # submission.to_csv("./output/0706_sub_NB_PL.csv", index=False)
    #
    # end_time = time.time()
    # print("Running time: %f s\n" % (end_time - start_time))

    index = pd.read_csv("./input/validation_new.csv", encoding="utf-8", usecols=[0])
    submission = pd.read_csv("./output/0706_sub_NB_PL.csv", encoding="utf-8")
    ...
    submission.to_csv("./output/0706_result_NB_PL.csv", index=False, header=None)
